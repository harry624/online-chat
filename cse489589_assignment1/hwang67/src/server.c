...
int showStatistics(){
    ...
    for (int i = 0; i < hisClientscnt; i++){
        ...
        }else if (hisClients[i].islogIn == 0){
        ...
        if(FD_ISSET(STDIN, &read_fds)){
            ...
            if (!(strcmp(argv[0], "AUTHOR"))){
              ...
              cse4589_print_and_log("[%s:END]\n", argv[0]);
                ...
                int k;
                for(k = 0; strcmp(hisClients[k].hostIP, argv[1]) && k < 4; k++);

                if(checkIPVaildation(argv[1]) == 0 && k < 4){
                    cse4589_print_and_log("[%s:SUCCESS]\n", argv[0]);
                          ...
                          */
                          if(hisClients[k].bufferedmsg[0] != NULL){
                              for (int n = 0; hisClients[k].bufferedmsg[n] != NULL && n < 100; n++){
                                  ...
                                  char *msghandler;
                                  char temp[310];
                                  strcpy(temp, hisClients[k].bufferedmsg[n]);
                                  msghandler = strtok(temp, ":*");
                                  char ip[20], msg[256];
                    ...
                    if((nbytes = recv(i, buf, sizeof buf, 0)) <= 0){
                        ...
                        for(int k = 0; k < hisClientscnt; k++){
                            if(!strcmp(hisClients[k].hostIP, senderIP)){
                                ...
                                // cse4589_print_and_log("hostIP:%s, senderIP: %s, %d\n", hisClients[k].hostIP, senderIP, hisClients[k].islogIn);
                                break;
                            }
                        }
                        close(i);             //close fd
                              ...
                              memset(&msg, 0 ,sizeof(msg));
                                ...
                                for (j = 1; j <= fdmax; j++){
                                  ...
                                  if(bytes_sent == 0 && isBlocked == 0){
                                    ...
                                    hisClients[k].bufferedmsg[n] = malloc(310);
                                  ...
                                  for (j = 1; j <= fdmax; j++){
                                      ...
                                      if (FD_ISSET(j, &master)){
                                          if (j != listenfd && j != i){
                                              int sendingres = send(j, msgsent, nbytes, 0);
                                              if(sendingres == -1){
                                                perror("send");
                                              }else{
                                                  //receiver count
